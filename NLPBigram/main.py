...
def print_best_sentence_with_probs(lm, context_word: str):
    """
    :param lm:
    :param context_word: An initial word to seed the sentence with
    :return: The sentence generated by repeatedly taking the best next word
    """
    sentence = [context_word]
    for i in range(0, 10):
        next_word = get_best_word(lm, context_word)
        prob = lm.get_probability(context_word, next_word)
        print("best:", context_word, "->", next_word, "p=", prob)
        context_word = next_word
        sentence.append(next_word)
        if next_word == END_SYMBOL:
            break
...
if __name__ == "__main__":
    ...
    prefixes = ["I", "You", "We", "He", "She"]
    for prefix in prefixes:
        print("best:", prefix, repr(get_best_sentence(lm, prefix)))
        print("sample:", prefix, repr(sample_sentence(lm, prefix)))
    repeat_prefix = "I"
    for i in range(0, 3):
        print("sample", repeat_prefix, i + 1, repr(sample_sentence(lm, repeat_prefix)))
